...
function buildMuleSoftBPMN(artifact, platformData, iflowId, iflowName, senderAdapter, receiverAdapter) {
  ...
  // buildStepsFromProcessors templates use SEQ_TO_{id} as a placeholder, but the
  // actual sequenceFlow elements generated by buildSequenceFlowsFromSteps use:
  //   SEQ_Start_To_{first.id}  for step[0]
  //   SEQ_FROM_{prev.id}       for step[i>0]   (SEQ_FROM_{prev.id}_DEFAULT for routers)
  for (let i = 0; i < steps.length; i++) {
    ...
    steps[i].xml = steps[i].xml.replace(
      `<bpmn2:incoming>SEQ_TO_${steps[i].id}</bpmn2:incoming>`,
...
// ── Derive receiver adapter type from connectorTypes / primary_connector ─────
function deriveReceiverAdapterType(pd, artifact) {
  const connTypes = (pd.connectorTypes || []).map(c => String(c).toLowerCase());
  const primary   = (artifact.primary_connector || '').toLowerCase();

  if (connTypes.some(c => c.includes('salesforce') || c.includes('sfdc')) || primary.includes('salesforce')) return 'salesforce';
  if (connTypes.some(c => c.includes('jdbc') || c.includes('database') || c.includes('db:')) || primary.includes('database') || primary.includes('jdbc')) return 'jdbc';
  if (connTypes.some(c => c.includes('jms') || c.includes('mq')) || primary.includes('jms')) return 'jms';
  if (connTypes.some(c => c.includes('sftp') || c.includes('ftp')) || primary.includes('sftp') || primary.includes('ftp')) return 'sftp';
  if (connTypes.some(c => c.includes('sap') || c.includes('rfc') || c.includes('idoc')) || primary.includes('sap')) return 'rfc';
  if (connTypes.some(c => c.includes('soap') || c.includes('wsdl') || c.includes('cxf')) || primary.includes('soap')) return 'soap';
  if (connTypes.some(c => c.includes('mail') || c.includes('smtp')) || primary.includes('mail') || primary.includes('smtp')) return 'mail';
  return 'http';
}
...
function buildReceiverExtensionsFromConfig(pd, artifact) {
  ...
  const adapterType = rc ? rc.type.toLowerCase() : deriveReceiverAdapterType(pd, artifact);
  return `<ifl:property><key>adapterType</key><value>${adapterType}</value></ifl:property>`;
}

function buildReceiverChannelFromConfig(pd, artifact) {
  const rc = pd.receiverConfigs && pd.receiverConfigs[0];
  if (!rc) {
    // Derive adapter type from connectorTypes / primary_connector instead of always defaulting to http
    const adapterType = deriveReceiverAdapterType(pd, artifact);
    if (adapterType === 'jdbc') {
    ...
    if (adapterType === 'salesforce') {
      return `<ifl:property><key>adapterType</key><value>salesforce</value></ifl:property>
        ...
        <ifl:property><key>operation</key><value>query</value></ifl:property>`;
    }
    if (adapterType === 'jms') {
      return `<ifl:property><key>adapterType</key><value>jms</value></ifl:property>
    ...
    if (adapterType === 'rfc' || adapterType === 'sap') {
      return `<ifl:property><key>adapterType</key><value>rfc</value></ifl:property>
        <ifl:property><key>sapHost</key><value>{{param.receiver.sap.rfc.host}}</value></ifl:property>
        <ifl:property><key>functionName</key><value>{{SAP_FUNCTION}}</value></ifl:property>`;
    }
    if (adapterType === 'soap') {
